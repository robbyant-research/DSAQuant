"""
Single-GPU inference script for Wan2.1-T2V-14B (text-to-video).
...
# ---- Main switches ----
USE_QUANTIZATION = _env_bool("USE_QUANTIZATION", True) # False = FP16 inference; True = W4A4 quantized inference

# Pretrained model path (Diffusers format)
PRETRAINED_MODEL_PATH = os.environ.get("PRETRAINED_MODEL_PATH", "/path/to/Wan2.1-T2V-14B-Diffusers")
...
QUANT_CHECKPOINT_PATH = os.environ.get("QUANT_CHECKPOINT_PATH", "/path/to/wan14b_w4a4_checkpoint")

# Output directory
OUTPUT_DIR = os.environ.get("OUTPUT_DIR", "outputs/wan2.1_14b")

# Inference settings
PROMPT = os.environ.get("PROMPT", "A cat sitting on a windowsill watching the rain outside.")
HEIGHT = 480
WIDTH = 832
NUM_FRAMES = 81
NUM_INFERENCE_STEPS = int(os.environ.get("NUM_INFERENCE_STEPS", "50"))
GUIDANCE_SCALE = 5.0
FLOW_SHIFT = 3.0  # Recommended for the 14B model: 5.0 at 720p and 3.0 at 480p.
...
def build_model_quantized():
    ...
    # model is already on target dtype/device for the 14B memory profile
...
def precompute_text_embeddings(device):
    ...
    This prevents the text encoder (~10 GB) and transformer (~28 GB) from occupying GPU memory simultaneously.
    """
...
def run_inference(model, model_args, device, tokenizer=None, prompt_embeds=None, negative_prompt_embeds=None):
    ...
    if tokenizer is not None and prompt_embeds is not None and negative_prompt_embeds is not None:
        # Use precomputed text embeddings without a text encoder.
...
def main():
    ...
    print("=" * 60)
    print(f"Single-GPU DSAQuant Inference - Wan2.1-T2V-14B - {mode_str}")
    print("=" * 60)
    ...
    # Release the text encoder (~10 GB) immediately so it never resides alongside the transformer (~28 GB).
    tokenizer = None
    prompt_embeds = None
    ...
    # Step 3: Run inference; the VAE is loaded lazily inside the pipeline.
